...
FROM
	{0} WHERE 1=1 {1}",tableName,where);
        ...
        public DataTable ExcuteProc(string procName,string where)
        {
            SqlParameter x1 = new SqlParameter("@sTable", SqlDbType.NChar, 40);
            x1.Value = where.Trim();
            SqlParameter[] param = new SqlParameter[] { x1 };
        ...
        public DataTable GetInitialRecords(string tplCode)
        {
            DataTable dt = idb.RunProcReturnDatatable("qx_tech_InitialRecord", new List<SqlParameter>() { new SqlParameter("@tplCode", tplCode) }.ToArray());
            return dt;
        }
    }
}
