...
namespace QX.Common.C_Class
{
    public class BindModelHelper
    {
        ...
        public void SetDropDownStyle(ComboBox cb)
        {
            cb.DropDownStyle = ComboBoxStyle.DropDown;
            cb.Leave += new EventHandler(Combox_Leave);
        }
        ...
        void Combox_Leave(object sender, EventArgs e)
        {
            ...
            string value = cb.Text.ToString();

            int result = 0;
            if (int.TryParse(value, out result))
            {
                if (cb.Items.Count <= result)
                {
                    cb.SelectedItem = cb.Items.Count;
                }
                else
                {
                    cb.SelectedItem = result;
                }
            }
            else
            {
