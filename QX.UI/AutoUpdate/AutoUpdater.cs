...
namespace QX.UpdateHelper
{
    public class AutoUpdater
    {
        ...
        private void StartDownload(List<DownloadFileInfo> downloadList)
        {
            ...
            //{
            MessageBox.Show("程序需要重新启动才能应用更新，请点击确定重新启动程序。", "提示", MessageBoxButtons.OK);

            if (File.Exists(Path.Combine(Application.StartupPath, "QX.UpdateLoader1.exe")))
            {
                Process.Start(Path.Combine(Application.StartupPath, "QX.UpdateLoader1.exe"));   
            }
            else
            {
