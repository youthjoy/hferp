...
namespace QX.Framework.AutoForm
{
    public class FormHelper
    {
        ...
        void ctr_DoubleClick(object sender, EventArgs e)
        {
            ...
            SystemInformationHelp helper = new SystemInformationHelp(ct);
            helper.Show();
            return;
        }
        ...
        public void PermissionControl(Form form, string module)
        {
            ...
            PermissionControl(controls, module);

            //Control[] clist = form.Controls.Find("", true);
        ...
        /// <param name="module"></param>
        public void PermissionControl(ToolStripItemCollection tsItems,string module)
        {
            BLL.Bll_UserPermission upInstance = new QX.BLL.Bll_UserPermission();

            List<Sys_Function> funList = upInstance.GetFunctionByPareCode("2", module);

            var plist = upInstance.GetUserPermissionList(SessionConfig.UserName);

            foreach (var f in funList)
            {
                var temp = tsItems.Find(f.Fun_Code,true);
                ...
                if (plist.FirstOrDefault(o => o.PU_FunCode == f.Fun_Code) != null)
                {
                    temp[0].Enabled = true;
                ...
                {
                    if (QX.Shared.SessionConfig.IsAdmin)
                    {
                        temp[0].Enabled = true;
                    }
                    else
        ...
        /// <param name="module"></param>
        public void PermissionControl(Control.ControlCollection controls, string module)
        {
            BLL.Bll_UserPermission upInstance = new QX.BLL.Bll_UserPermission();

            List<Sys_Function> funList = upInstance.GetFunctionByPareCode("2", module);

            var plist = upInstance.GetUserPermissionList(SessionConfig.UserCode);

            foreach (var f in funList)
            {
                Control[] temp = controls.Find(f.Fun_Code, true);
